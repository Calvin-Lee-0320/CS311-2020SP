        ...
        if(V->Right == NULL && V->Left == NULL)
          {
            ...
            // ** if V is a left child of P
              if(P->Left == V){delete V;P->Left = NULL;}
              // ** delete V and also make Parent's left NULL
        
              climbup(P);// *** EC call here from P to adjust height and B
         ...
         { Vertex* C = V->Left; // the left child
             ...
             if(P->Left == V){P->Left = V->Left;}  // ** You need if then else to determine Parent's left or right
             else{P->Right = V->Left;}//    should point to V's left child;
             (V->Left)->Up = P;// ** Make V's left child point UP to the parent;
             ...
             { Vertex* C = V->Right;  // the right child
                 ...
                 else{P->Right = V->Right;}// ** You need if then else to determine Parent's left or right
                  //    should point to V's right child;
                 (V->Right)->Up = P;// ** Make V's right child point UP to the parent;
                  cout << C->Elem << " points up to " << C->Up->Elem << endl;
                 delete V;// ** Be sure to delete V
                 climbup(P);// *** EC call from P to adjust height and BF
